...
// ====================================================================
// WebCam Proctoring Component (reuses interview proctoring endpoint)
// ====================================================================
    ...
    <div className="w-full max-w-lg mx-auto">
      <div className="bg-white/80 backdrop-blur-sm border border-gray-200/50 rounded-2xl p-6 shadow-xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold bg-gradient-to-r from-amber-800 to-orange-800 bg-clip-text text-transparent">Proctoring Monitor</h2>
        ...
        <div className="space-y-4">
          <div className="relative">
            <div className={`relative aspect-video bg-gray-900 rounded-xl overflow-hidden transition-all duration-300 ${proctorData.violation_detected ? 'ring-4 ring-red-400 ring-opacity-50 shadow-lg shadow-red-400/20' : 'ring-2 ring-amber-200 ring-opacity-30'}`}>
              ...
              {!isActive && !error && <div className="absolute inset-0 bg-gradient-to-br from-amber-900/80 to-orange-900/80 flex items-center justify-center"><div className="text-center"><div className="relative"><Camera size={48} className="text-white/70 mx-auto mb-3" /><div className="absolute inset-0 animate-ping"><Camera size={48} className="text-white/30 mx-auto" /></div></div><p className="text-white/90 font-medium">Initializing AI Monitor...</p></div></div>}
            </div>
...
// ====================================================================
// HR Behavioral Interview Question Labels
// ====================================================================
const questionLabels = {
  hr_intro: 'Career Motivation',
  hr_conflict: 'Conflict Resolution',
  hr_collaboration: 'Collaboration',
  hr_leadership: 'Leadership',
  hr_adaptability: 'Adaptability',
  hr_situational: 'Situational Judgment',
};

// ====================================================================
// Main HR Interview Component
// ====================================================================
...
export default function HRInterviewDashboard() {
  ...
  const apiUrl = 'http://localhost:5000/hrround/api';
  ...
  const startInterview = async () => {
    try {
      ...
      setError(`Failed to start HR interview: ${err.message}`);
      console.error('Error starting HR interview:', err);
    }
  };
  ...
  const finishInterview = async () => {
    if (!window.confirm("Are you sure you want to finish the HR interview? Your progress will be evaluated.")) return;
    ...
    try {
      ...
      setError(`Failed to finish HR interview: ${err.message}`);
      console.error('Error finishing HR interview:', err);
    } finally {
  ...
  const endExamSession = async (evaluationText) => {
    try {
      ...
      const resultResponse = await fetch(`${resultsApiUrl}/round4/results`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!resultResponse.ok) throw new Error(`Failed to submit HR results: ${resultResponse.statusText}`);
    } catch (err) {
      console.error('Error ending HR exam session:', err);
    }
  };
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl p-8 max-w-2xl w-full border border-white/20 transition-all duration-500 hover:shadow-3xl hover:-translate-y-1">
          <div className="text-center mb-8">
            <div className="relative inline-block">
              <Award className="w-20 h-20 text-amber-500 mx-auto mb-6 animate-bounce" />
              <div className="absolute inset-0 bg-amber-100 rounded-full opacity-0 animate-[ping_1s_cubic-bezier(0,0,0.2,1)_forwards]" />
            </div>
            ...
            <p className="text-gray-500 text-lg">Round 4 — Behavioral Assessment</p>
            <div className="h-1 bg-gradient-to-r from-amber-100 to-orange-100 rounded-full w-3/4 mx-auto mb-8 mt-4" />
          </div>
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border border-gray-100 shadow-inner transition-all duration-300 hover:shadow-md">
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-xl font-semibold text-gray-800">
                Behavioral Evaluation Complete
              </h3>
            </div>
            ...
            <div className="space-y-4 text-gray-700 leading-relaxed">
              ...
              <div className="bg-amber-50/50 border-l-4 border-amber-400 p-4 rounded-r-lg">
                <p>
                  Your behavioral competencies have been assessed across leadership, collaboration, conflict resolution, adaptability, and situational judgment.
                </p>
              </div>
          ...
          <div className="mt-8 text-center">
            <div className="inline-flex items-center bg-gradient-to-r from-amber-100 to-orange-100 px-6 py-3 rounded-full shadow-sm">
          ...
          <div className="mt-12 text-center">
            ...
            <div className="mt-8">
              <button
                onClick={() => navigate('/')}
                className="px-8 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-semibold text-lg hover:scale-105 transition-all duration-200 shadow-lg shadow-amber-500/25"
              >
                Exit to Home Page
              </button>
            </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-orange-50 flex items-center justify-center p-4">
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl p-8 max-w-2xl text-center border border-white/20">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-bold bg-gradient-to-r from-amber-800 to-orange-800 bg-clip-text text-transparent mb-3">
              Welcome to your HR Behavioral Interview
            </h1>
            <p className="text-gray-600 leading-relaxed">
              This round evaluates your behavioral competencies including conflict resolution, leadership,
              collaboration, adaptability, and situational judgment. The AI interviewer will ask
              contextual follow-up questions based on your responses.
            </p>
            <div className="mt-4 inline-flex items-center bg-amber-100 px-4 py-2 rounded-full">
              <Users className="w-4 h-4 text-amber-700 mr-2" />
              <span className="text-sm font-medium text-amber-700">Round 4 — Behavioral Assessment</span>
            </div>
          </div>
          ...
          <button onClick={startInterview} className="px-8 py-4 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-medium hover:scale-105 transition-all duration-200 shadow-lg shadow-amber-500/25">
            <Play className="w-5 h-5 inline mr-2" />Start HR Interview
          </button>
        </div>
      </div>
    );
  ...
  // ====================================================================
  const currentLabel = questionLabels[questionStatus] || 'Behavioral Question';
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold bg-gradient-to-r from-amber-800 to-orange-800 bg-clip-text text-transparent">HR Interview</h1>
            <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-amber-100 rounded-full">
              <Clock className="w-4 h-4 text-amber-600" />
              <span className="text-sm font-medium text-amber-700">{formatTime(timeElapsed)}</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-sm text-gray-600">{currentLabel}</div>
            <div className="px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-sm font-medium">Question {questionNumber}/6</div>
          </div>
        </div>
      </header>
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-sm font-medium text-amber-600">{currentLabel}</span>
                    <span className="text-xs text-gray-500">Question {questionNumber}</span>
                  </div>
                  <h2 className="text-lg font-semibold text-gray-900 leading-relaxed">{currentQuestion}</h2>
                </div>
                <button onClick={() => speakText(currentQuestion)} disabled={isSpeaking} className={`ml-4 p-3 rounded-xl transition-all duration-200 ${isSpeaking ? 'bg-gradient-to-r from-amber-100 to-orange-100 text-amber-600 scale-105' : 'bg-white hover:bg-gray-50 text-gray-600 hover:scale-105'} shadow-md border border-gray-200`} title="Read question aloud">{isSpeaking ? <Pause className="w-5 h-5" /> : <Volume2 className="w-5 h-5" />}</button>
              </div>
            </section>
            ...
            <section className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 shadow-lg">
              ...
              <div className="relative">
                <textarea
                  ...
                  placeholder="Share your experience... You can type your answer here, or use the voice recording button..."
                  className="w-full min-h-[250px] bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border-2 border-dashed border-gray-200 focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all"
              ...
              <div className="flex justify-between items-center mt-6">
                <button onClick={finishInterview} disabled={isSubmitting || isFinishing} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-xl font-medium transition-all duration-200 disabled:opacity-50 hover:scale-105 shadow-lg shadow-red-500/25">
                  <XCircle className="w-4 h-4" />{isFinishing ? 'Finishing...' : 'Finish Interview'}
                </button>
                <div className="flex gap-4">
                  ...
                  <button onClick={submitAnswer} disabled={!transcript || isSubmitting || isFinishing} className="flex items-center gap-2 px-6 py-2 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-medium transition-all duration-200 disabled:opacity-50 hover:scale-105 shadow-lg shadow-amber-500/25">
                    {isSubmitting ? 'Submitting...' : 'Submit & Next'}<Send className="w-4 h-4" />
                  </button>
                </div>
              </div>
            </section>
