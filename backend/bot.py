...
class EnhancedRAGSystem:
    ...
    def extract_text_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            logger.error(f"Error in extract_text_node: {str(e)}")
            state["raw_text"] = []
            return state
    ...
    def _create_semantic_chunks(self, text: str, page_num: int) -> List[Dict]:
        ...
        for sentence in sentences:
            ...
            if current_length + sentence_length > 80:  # Reduced optimal chunk size
                if current_chunk:
...
def upload_pdf():
    ...
    try:
        ...
        if file_size > 10 * 1024 * 1024:
            return jsonify({'error': 'PDF file too large. Max size is 10MB.'}), 400
