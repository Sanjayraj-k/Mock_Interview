...
def retrieve_docs(state: AgentState):
    ...
    # Handle greetings without retrieval to prevent context dumping
    clean_query = original_query.strip().lower().rstrip("!.,?")
    if clean_query in ["hi", "hello", "hey", "greetings", "good morning", "good evening","ok","Thank You","Thank you","Thank you!","Thank you!","Bye","bye!"]:
        prompt = f"The user said: '{original_query}'. You are Shimal Akmal's portfolio assistant. Respond with a warm, professional greeting and briefly invite them to ask about his projects, skills, or experience. Do NOT list any portfolio details yet."
        return {"messages": [HumanMessage(content=prompt)]}
    # Handle basic conversation
    if clean_query in ["ok", "thank you", "thanks", "bye", "goodbye"]:
         return {"messages": [HumanMessage(content=f"The user said '{original_query}'. Respond politely to close or acknowledge the interaction.")]}

    # Handle specific hardcoded queries loose match
    if "cgpa" in clean_query:
        prompt = f"The user asked: '{original_query}'. You are Shimal Akmal's portfolio assistant. State that his CGPA is 8.0."
        return {"messages": [HumanMessage(content=prompt)]}
    
    if "gpa" in clean_query:
        # Note: If they ask for CGPA, the above block catches it. This is for if they strictly say 'gpa' without 'c'.
        prompt = f"The user asked: '{original_query}'. You are Shimal Akmal's portfolio assistant. State that his GPA is 8.3."
        return {"messages": [HumanMessage(content=prompt)]}
    ...
    augmented_prompt = f"""
You are an assistant for shimal akmal's portfolio. Always respond in a concise, summarized, and easy-to-understand format for users. Use simple language, short sentences, bullet points, and bold headings. Avoid long walls of text—focus on key highlights.
Context Information:
{context_text}
User Question:
{original_query}
