...
def _generate_evaluation_and_cleanup(memory):
    ...
    if not history.strip() or "Human:" not in history:
...
def finish_interview():
    try:
        ...
        # BUG FIX: Process the final answer sent from the frontend
        if request.json:
            user_answer = request.json.get('answer', '')
            if user_answer and user_answer.strip():
                history = memory.buffer_as_str
                # Check if there is a pending question to attach the answer to
                if "Assistant:" in history and "Human:" not in history.rsplit("Assistant:", 1)[1]:
                    last_question = history.rsplit("Assistant:", 1)[1].strip()
                    memory.save_context({"input": last_question}, {"output": user_answer})
                    logger.debug(f"Saved final answer before evaluation: {user_answer[:50]}...")
...
if __name__ == "__main__":
    try:
        # FIXED: Changed port from 6000 to 8000 and updated log message
