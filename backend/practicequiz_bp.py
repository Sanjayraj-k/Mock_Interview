"""
PracticeQuiz Blueprint for Flask Application
Handles quiz generation from various content sources (PDF, YouTube, Audio)
"""
...
# Blueprint for PracticeQuiz
practicequiz_bp = Blueprint('practicequiz', __name__, url_prefix='/practicequiz')
CORS(practicequiz_bp)
...
def preprocess_text(content: str, llm) -> str:
    """
...
def process_document(file_path: str, llm, embeddings) -> MultiQueryRetriever:
    """
    ...
    try:
        ...
        content = preprocess_text(content, llm)
...
def process_youtube_url(url: str, llm, embeddings) -> MultiQueryRetriever:
    """
...
def process_audio(file_path: str, llm, embeddings) -> MultiQueryRetriever:
    """
...
def generate_questions(state: GraphState, llm) -> GraphState:
    try:
...
@practicequiz_bp.route('/api/generate-quiz', methods=['POST'])
def generate_quiz():
    try:
        ...
        try:
            if content_type == "pdf":
                ...
                file.save(file_path)
                retriever = process_document(file_path, llm, embeddings)
            elif content_type == "youtube":
                youtube_url = request.form.get('youtube_url')
                if not youtube_url:
                    return jsonify({"error": "No YouTube URL provided"}), 400
                logger.info(f"Processing YouTube URL: {youtube_url}")
                retriever = process_youtube_url(youtube_url, llm, embeddings)
                ...
                file_path = os.path.join(upload_folder, file.filename)
                file.save(file_path)
                retriever = process_audio(file_path, llm, embeddings)
            else:
                return jsonify({"error": "Invalid content type. Use 'pdf', 'youtube', or 'audio'"}), 400

            quiz_graph = create_quiz_graph(llm)
        ...
        return jsonify({"error": "Failed to initialize AI services"}), 500

@practicequiz_bp.route('/health', methods=['GET'])
def health_check():
    return jsonify({
        "status": "healthy",
        "service": "practicequiz",
        "message": "PracticeQuiz service is running"
    }), 200