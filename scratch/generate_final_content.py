...
# 1. Academic Paper Content (6-7 pages dense text, Student & Teacher centric)
paper_content = """MockAI: A Multi-Agent LLM Framework for University Placement Preparation with Adaptive Aptitude Testing, Sandboxed Coding Compilation, and Gaze-Based Proctoring
...
where $\beta \geq 0$ is a tuning parameter controlling the severity of adaptivity (default $\beta = 2.0$). Topics with higher error rates receive a higher probability of selection, prompting the student to practice weaker topics.
...
# 2. College & Student-Teacher focused README file
readme_content = """# 🎯 MockAI - AI-Powered Placement Assessment and Mock Interview Platform
...
### 👨‍🎓 Student Journey
1. **Login** $\rightarrow$ Student authenticates via `StudentLandingpage`.
2. **Face Verification** $\rightarrow$ Align face with the camera to open the test.
3. **Assessment Rounds** $\rightarrow$ Complete Aptitude, Coding, Technical, and HR interviews. Gaze tracker monitors behavior.
4. **Scoring & Transcript** $\rightarrow$ View detailed scores, strengths, weaknesses, and feedback.

### 👩‍🏫 Teacher Journey
1. **Login** $\rightarrow$ Access dashboard via `/hr/login` (Teacher credentials).
2. **Role Creation** $\rightarrow$ Configure placement drive templates and requirements.
3. **Student Enrolment** $\rightarrow$ Add student list and assign placement practice papers.
4. **Analytics Monitoring** $\rightarrow$ Review aggregate placement readiness metrics, transcripts, and proctoring warnings.
...
cd backend
python -m venv env
# Windows: env\Scripts\activate | MacOS: source env/bin/activate
...
if __name__ == "__main__":
    # Path to paper
    paper_path = r"d:\mockai\scratch\paper_updated.txt"
    os.makedirs(os.path.dirname(paper_path), exist_ok=True)
    with open(paper_path, "w", encoding="utf-8") as f:
        f.write(paper_content.strip())
    print(f"Paper text generated at {paper_path}")

    # Path to README.md
    readme_path = r"d:\mockai\README.md"
